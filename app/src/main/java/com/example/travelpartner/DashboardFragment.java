    ...
    private void setupIndicator() {
        if (viewPager != null && viewPager.getAdapter() != null) {
            ...
            for (int i = 0; i < slideCount; i++) {
                ...
                indicators[i].setImageResource(R.drawable.unselected_indicator); // Set the unselected indicator drawable
                LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
                        LinearLayout.LayoutParams.WRAP_CONTENT,
                        LinearLayout.LayoutParams.WRAP_CONTENT
                );
                params.setMargins(8, 0, 8, 0); // Set margins between indicators
    ...
    private void setCurrentIndicator(int position) {
        if (indicators != null) {
            for (int i = 0; i < indicators.length; i++) {
                if (i == position) {
                    indicators[i].setImageResource(R.drawable.selected_indicator); // Set the selected indicator drawable
                } else {
                    indicators[i].setImageResource(R.drawable.unselected_indicator); // Set the unselected indicator drawable
                }
            }
